...
Bootstrap为了简化Handler的编排，提供了Cbannellnitializer，它继承了ChannelHandlerAdapter，当TCP链路注册成功之后，调用initChannel接口，用于设置用户ChannelHandler。
...
由于写操作不修改readerIndex指针，读操作不修改writerlndex指针，因此读写之间不再衙要调整位置指针，这极大地简化了缓冲区的读写操作，避免了由于遗漏或者不熟悉flip()操作导致的功能异常。
...
Arena本身是指一块区域，在内存管理中，Memory Arena是指内存中的—大块连续的区域，PoolArena就是Netty的内存池实现类。
...
* JDK的SocketChannel和ServerSocketChannel的主要职责就是网络I/O操作，由于它们是SPI类接口，由具体的虚拟机厂家来提供，所以通过继承SPI功能类来扩展其功能的难度很大；直接实现ServerSocketCbannel和SocketChannel抽象类，其工作量和重新开发一个新的Channel功能类是差不多的。
...
* Channel接口的定义尽量大而全，为SocketChanoel和ServerSocketCbannel提供统一的视图，由不同子类实现不同的功能，公共功能在抽象父类中实现，最大程度上实现功能和接口的重用。
...
13. CbannelFuture bind(SocketAddress localAddress)
	* 绑定指定的本地Socket地址localAddress,该方法会级联触发ChanneIHandler.bind(ChannelHandlerContext,SocketAddress,ChannelPromise)事件。
...
它返回Channelld对象，CbannelId是Channel的唯一标识，它的可能生成策略：
...
Netty基千事件驱动，我们也可以理解为当Cbnanel进行I/O操作时会产生对应的I/O事件，然后驱动事件在CbannelPipeline中传播，由对应的CbannelHandler对事件进行拦截和处理，不关心的事件可以直接忽略。
...
* 由于NIO Channel、NioSocketChannel和NioServerSocketCbannel需要共用，所以定义了一个java.nio.SocketChannel和java.nio.ServerSocketCbanneI的公共父类SelectableChannel，用于设置SelectableChannel参数和进行I/O操作。  
...
doBind方法，对于NioSocketCbannel和NioServerSocketCbannel有不同的实现。
...
* 首先，对当前索引做步进缩减，然后获取收缩后索引对应的容量，与实际读取的字节数进行比对，如果发现小千收缩后的容量，则重新对当前索引进行赋值，取收缩后的索引和最小索引中的较大者作为最新的索引。
...
* 不同的应用场景，传输的码流大小千差万别，无论初始化分配的是32K还是IM，都会随着应用场景的变化而变得不适应。
...
* 更节约内存。
	* 假如通常情况下请求消息平均值为1M左右，接收缓冲区大小为1.2M；突然某个客户发送了一个IOM的流媒体附件，接收缓冲区扩张为IOM以接纳该附件，如果缓冲区不能收缩，每次缓冲区创建都会分配IOM的内存，但是后续所有的消息都是lM左右，这样会导致内存的浪费，如果并发客户端过多，可能会发生内存溢出，最终宅机。

### ChannelPipeline和ChannelHandler

Netty的ChannelPipeline和ChannelHandler机制类似千Serviet和Filter过滤器，这类拦截器实际上是职责链模式的一种变形，主要是为了方便事件的拦截和用户业务逻辑的定制。

Netty的Channel过滤器实现原理与Servlet Filter机制一致，它将Channel的数据管道抽象为ChannelPipeline，消息在ChannelPipeline中流动和传递。
CbannelPipeline持有I/O事件拦截器ChannelHandler的链表，由ChannelHandler对I/O事件进行拦截和处理，
...
ChannelPipeline是线程安全的，这意味着N个业务线程可以并发地操作CbannelPipeline而不存在多线程并发问题。  
...
由千ChannelHandler是Netty框架和用户代码的主要扩展和定制点，所以它的子类种类繁多、功能各异。
...
```java
// 达到需要忽略的字节数后对discardingTooLongFrame等进行置位
