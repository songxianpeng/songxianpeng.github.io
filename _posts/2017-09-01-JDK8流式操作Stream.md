...
```java
// tryAdvance() 方法尝试处理单个元素。如果没有元素，tryAdvance() 只会返回 false；否则，它会前移游标，将当前元素传递给所提供的处理函数并返回 true。
...
<img src="/static/img/JDK8流式操作Stream/stream.svg" class="ibm-downsize" alt="一个流机器的动画图像。" height="339" width="454">
